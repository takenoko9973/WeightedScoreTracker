...
pub fn draw(ctx: &egui::Context, data: &AppData, state: &mut UiState) -> Option<Action> {
    ...
    draw_add_category_window(ctx, state)
        .or_else(|| draw_edit_decay_window(ctx, state))
        .or_else(|| draw_delete_category_confirm(ctx, state))
        .or_else(|| draw_delete_score_confirm(ctx, data, state))
}
